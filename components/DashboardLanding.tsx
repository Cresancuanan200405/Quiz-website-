...
import { categoryMeta, onlinePlayers } from "@/lib/mockData";
...
export default function DashboardLanding() {
  ...
  const weeklyStats = useMemo(() => {
    ...
    const weeklyTargetPoints = Math.max(500, quizzesCompleted * 150);
  ...
  }, [now, quizHistory, quizzesCompleted]);
        ...
        <div className="mx-auto w-full max-w-6xl space-y-5">
          <section className="relative overflow-hidden rounded-card border border-black/10 bg-white/70 p-6 shadow-[0_16px_42px_rgba(15,23,42,0.16)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/70 dark:shadow-[0_20px_56px_rgba(2,8,32,0.5)]">
            ...
            <div className="relative flex flex-wrap items-center justify-between gap-5">
              ...
              <Link
                href="/quiz?instant=1&count=15&random=1"
          ...
          <section className="grid items-start gap-4 lg:grid-cols-2">
            <div className="space-y-3">
              ...
              {hasNoQuizRecords ? <p className="text-xs text-[var(--text-secondary)]">No quiz records yet. Play a quiz to populate your dashboard stats.</p> : null}
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="glass rounded-card border border-black/10 bg-gradient-to-br from-white/78 via-white/60 to-cyan-100/25 p-4 shadow-[0_16px_28px_rgba(15,23,42,0.1)] dark:border-white/10 dark:bg-gradient-to-br dark:from-slate-900/70 dark:via-slate-900/50 dark:to-cyan-900/18 dark:shadow-[0_18px_32px_rgba(2,8,25,0.4)]">
                  ...
                  <div className="h-3 overflow-hidden rounded-full dark:bg-white/10 bg-black/5">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${weeklyStats.weeklyGoalProgress}%` }} className="h-full rounded-full bg-gradient-to-r from-violet-500 to-green-400" />
                  </div>
                  <p className="mt-2 text-xs text-[var(--text-secondary)]">{weeklyStats.weeklyPoints} / {weeklyStats.weeklyTargetPoints} points this week</p>
                </div>
                ...
                {unlockedAchievements.slice(0, 8).map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <motion.span aria-hidden="true" className="pointer-events-none absolute inset-0 rounded-xl" animate={{ opacity: [0.2, 0.5, 0.2] }} transition={{ duration: 2.2, repeat: Infinity, ease: "easeInOut" }} style={{ boxShadow: "0 0 12px rgba(99,102,241,0.35)" }} />
                    </motion.div>
                  );
                })}
              </div>
          ...
          <section className="grid items-start gap-4 lg:grid-cols-3">
            <article className={`glass rounded-card border border-black/10 p-4 shadow-[0_16px_30px_rgba(15,23,42,0.12)] transition-all hover:-translate-y-0.5 dark:border-white/10 dark:shadow-[0_18px_36px_rgba(2,8,25,0.42)] ${leaderboardMode === "battle" ? "bg-gradient-to-br from-white/80 via-white/60 to-cyan-100/30 hover:shadow-[0_22px_36px_rgba(34,211,238,0.18)] dark:bg-gradient-to-br dark:from-slate-900/72 dark:via-slate-900/52 dark:to-cyan-900/22" : "bg-gradient-to-br from-white/80 via-white/60 to-indigo-100/28 hover:shadow-[0_22px_36px_rgba(79,70,229,0.18)] dark:bg-gradient-to-br dark:from-slate-900/72 dark:via-slate-900/52 dark:to-indigo-900/22"} lg:col-span-2`}>
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="font-sora text-lg font-semibold dark:text-white text-gray-900">{leaderboardMode === "battle" ? "1v1 Battle Preview" : "Trivia Journey Preview"}</p>
                  <p className="text-xs text-[var(--text-secondary)]">{leaderboardMode === "battle" ? "Ranked by battle points" : "Ranked by journey points"}</p>
                </div>
                <div className="inline-flex rounded-full border border-black/10 bg-white/65 p-1 text-xs dark:border-white/10 dark:bg-white/5">
                  <button type="button" onClick={() => setLeaderboardMode("trivia")} className={`focus-ring rounded-full px-3 py-1 ${leaderboardMode === "trivia" ? "bg-violet-500/25 text-violet-700 dark:text-violet-100" : "text-[var(--text-secondary)]"}`}>Trivia Journey</button>
                  <button type="button" onClick={() => setLeaderboardMode("battle")} className={`focus-ring rounded-full px-3 py-1 ${leaderboardMode === "battle" ? "bg-cyan-500/25 text-cyan-700 dark:text-cyan-100" : "text-[var(--text-secondary)]"}`}>1v1 Battle</button>
                </div>
              </div>
              ...
              <div className="mt-3 text-right">
                <Link href={leaderboardMode === "battle" ? "/battle/leaderboard" : "/leaderboard"} className={`text-sm ${leaderboardMode === "battle" ? "text-cyan-700 hover:text-cyan-600 dark:text-cyan-200 dark:hover:text-cyan-100" : "text-violet-700 hover:text-violet-600 dark:text-violet-200 dark:hover:text-violet-100"}`}>
